...
class Newlink(object):
    def __init__(self, part, prefix_name):
        ...
        all_VP = set(open('./Infer_File/chooseVP' + self.part + '.txt', 'r').read().split('\n'))
        self.fullVP = set(open('./Infer_File/fullVP.txt', 'r').read().split('\n'))
        self.fullAS = eval(open('./Infer_File/chooseAS.txt', 'r').read())
        self.fullVP1 = all_VP & self.fullVP
        self.fullAS1 = self.fullAS - self.fullVP

        self.ba = BasicAtts('./Infer_File/asrel' + self.part + '.txt')
        self.ba.calPr()
    ...
    def getASpfx(self):
        with open(self.prefix_name) as f:
            for line in f:
                AS = line.strip().split(',')[-1]               # AS 和 prefix的映射表需要更新
                if '_' not in AS:
                    self.ASpfx[AS] += 1
                else:
    ...
    def getEdgeAmount(self):
        with open('./Infer_File/asprefix' + part + '.txt') as f:
            for line in f:
    ...
    def getTriple(self):
        with open('./Infer_File/triplet_miss' + self.part + '.txt') as f:
            for line in f:
                if line == '\n':
                    continue
    ...
    def assignPosition(self):
        ...
        report = 1
        with open('./Infer_File/asprefix' + self.part + '.txt') as f:
            for line in f:
                report += 1
                if report % 1000000 == 0:
                    print(report)
    ...
    def xgboostInfer(self):
        ...
        for VP in (self.fullVP | self.fullAS):
            if self.ba.getHierarchy(VP) in [5, 6]:          # 只考虑高层VP—— clique 和 high tier
                banVP.add(VP)
        ...
        for tri in self.cnt:
            ...
            if tri[2] in fullVP:
                ...
                x.append(1)
            x.append(self.cnt[tri])         # (asn1, asn2, asn3) 该边出现的次数
            ...
            x.extend([self.ba.pr[tri[0]]*10000, self.ba.pr[tri[1]]*10000, self.ba.pr[tri[2]]*10000])        # pr = PageRank
            x.extend([self.getNum(edgeamount[0]), self.getNum(edgeamount[1])])      # (asn1, asn2) 和 (asn2, asn3) 两条边出现次数
            x.extend([self.ba.getEdgeRelationship(tri[0], tri[1]), self.ba.getEdgeRelationship(tri[1], tri[2])])    # 根据AS-Rank得到的边的关系
            x.extend([self.ba.getHierarchy(tri[0]), self.ba.getHierarchy(tri[1]), self.ba.getHierarchy(tri[2])])    # 三个节点的层级
            x.extend([self.ba.getDegree(tri[0]), self.ba.getDegree(tri[1]), self.ba.getDegree(tri[2])])             # 三个节点的度
            ...
            x.extend([len_prev, len_next])          # (asn1, asn2, asn3)的前后出现的情况有多少种
            x.extend(rp)                            # (asn1, asn2, asn3)在路径中所处位置情况
            x.extend(rpn)                           # (asn1, asn2, asn3)是否处于路径的首位
            ...
            if tri[0] in test or tri[2] in test:                # 测试集
                ...
                if tri in self.triple:
                    ...
                    x.append(0.0)
                    attribute_test.append(x)
            elif tri[0] in fullVP or tri[2] in fullVP:          # 训练集
                if tri in self.triple:
                    c1 += 1
                    x.append(1.0)
        ...
        ftri = open('./Infer_File/futher' + self.part + '.txt', 'w')
        for i in range(len(predictions)):
            if predictions[i] == 1:
