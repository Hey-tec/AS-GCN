...
class UniquePrefix(object):
    ...
    def writePrefix(self):
        for choose in [0, 1]:
            fout = open('./Infer_File/chooseVP' + str(choose) + '.txt', 'w')
            for vp in self.vp_group[choose]:
                fout.write(vp + '\n')
            fout.close()

            fout = open('./Infer_File/aspaths' + str(choose) + '.txt', 'w')
            for path in self.path_group[choose]:
                fout.write(path + '\n')
            fout.close()

            fout = open('./Infer_File/asprefix' + str(choose) + '.txt', 'w')
            for prefix in self.prefix_group[choose]:
                fout.write(prefix + '\n')
            fout.close()
        fout = open('./Infer_File/fullVP.txt', 'w')
