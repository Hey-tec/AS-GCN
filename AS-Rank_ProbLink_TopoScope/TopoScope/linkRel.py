...
for name in ['0', '1']:
    with open('./Infer_File/triplet_miss' + name + '.txt') as f:
        for line in f:
            if line == '\n':
                continue
...
for name in ['0', '1']:
    with open('./Infer_File/futher' + name + '.txt') as f:
        for line in f:
            if line == '\n':
                continue
            t = line.strip().split('|')
...
fout = open('./Infer_File/asrel_hidden.txt', 'w')
