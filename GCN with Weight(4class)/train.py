...
def train(epoch):
    ...
    loss_val = F.cross_entropy(output[data.valid_mask], data.y[data.valid_mask])
...
def test(max_test_acc):
    ...
    if acc_test.item() > max_test_acc:
        ...
        torch.save(model.state_dict(), './Experiment Model/model_GCN_13.pt')
...
data = data.to(device)
model = ResGCN(data.x.shape[1], 4, data.edge_index, data.edge_attr).to(device)
...
Loss_Data_Log = open("./Experiment Log/model_GCN_13.txt", "w")
Loss_Data_Log.write("实验一：model 1*3      lr:0.1" + "\n")
